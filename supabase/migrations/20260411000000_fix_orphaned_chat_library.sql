-- Fix false-positive orphaned chat detection for library chats.
-- Library chats have NULL book_id and are not orphaned.
...
BEGIN
  ...
  -- Orphaned chats (book deleted but book_id was set)
  SELECT 'orphaned_chat', 'error',
         'Chat references deleted book', c.id, NULL::text
  FROM chats c
  LEFT JOIN books b ON c.book_id = b.id
  WHERE c.book_id IS NOT NULL AND b.id IS NULL
