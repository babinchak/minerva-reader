...
const CACHE_NAME = "minerva-reader-pwa-v2";
const EPUB_CACHE_NAME = "minerva-epub-content-v1";
const CORE_ASSETS = [
  "/",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    (async () => {
      ...
      const keep = new Set([CACHE_NAME, EPUB_CACHE_NAME]);
...
self.addEventListener("fetch", (event) => {
  ...
  // Cache EPUB content from Supabase Storage (stale-while-revalidate)
  if (
    url.origin !== self.location.origin &&
    url.pathname.includes("/readium-manifests/")
  ) {
    event.respondWith(
      (async () => {
